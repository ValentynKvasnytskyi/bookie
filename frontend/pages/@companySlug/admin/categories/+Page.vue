...
const categories: Ref<CategoryEntity[]> = ref(data.value.categories);
...
<template>
  <Card>
    <template #header>
      <span class="font-medium">{{ t("categories.title") }}</span>
      <a
        :href="getLocalizedUrl(`/${pageContext.routeParams.companySlug}/admin/categories/create`, pageContext.locale)"
        class="bg-green-600 text-white px-3 py-1 rounded-2 hover:bg-green-700"
        >{{ t("button.create") }}</a
      >
    </template>
    <template #body>
      <Table :columns="columns" :items="categories" :totalItems="data.totalItemsCount">
        ...
        <template #actions="{ item }">
          <CButtonGroup vertical>
            <a
              :href="
                getLocalizedUrl(
                  `/${pageContext.routeParams.companySlug}/admin/categories/${item._id}`,
                  pageContext.locale,
                )
              "
